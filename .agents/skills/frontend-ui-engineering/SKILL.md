...
### Construct v2 — glass controls

When working under `apps/web/`:

- Read [`apps/web/docs/glass-surfaces.md`](../../../apps/web/docs/glass-surfaces.md) before styling overlays.
- **Never ship native `<select>`.** Use `GlassSelect` / `GlassListboxPortal` (`src/components/ui/glass-select.tsx`) so dropdown panels use frosted `glass-portaled-popover` instead of OS chrome.
- **Never ship `window.alert` / `confirm` / `prompt`.** Use `ConfirmDialog` or toasts (`apps/web/docs/glass-surfaces.md`).
