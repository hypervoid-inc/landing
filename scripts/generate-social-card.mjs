...
async function studioHeadline(lines, accentFrom) {
  ...
  const placed = await Promise.all(
    lines.map((line, index) =>
      place({
        ...
        fill: index >= accentFrom ? BRAND_STRONG : INK,
      }),
    ),
  );
...
async function studioBadge(eyebrow) {
  ...
  return {
    ...
    white: rect("#ffffff", 4) + text.draw("#ffffff"),
  };
}

async function studioLine(text, style, { baseline, cap, tracking }, fill) {
  ...
  return { cap, solid: placed.markup, white: placed.draw("#ffffff") };
}
...
async function studioTypeLayer({
  ...
  const blocks = [
    ...
    await studioBadge(eyebrow),
    await studioHeadline(headline, accentFrom),
    await studioLine(subline, TEXT, STUDIO_TYPE.subline, INK_MUTED),
    await studioLine("construct.computer", TEXT, STUDIO_TYPE.domain, INK_MUTED),
...
const studio = (card.theme ?? "studio") === "studio";
...
for (let candidate = 1; candidate <= candidates; candidate += 1) {
  ...
  const result = await generateImage({
    ...
    references: studio ? posterReferences() : referenceImages,
